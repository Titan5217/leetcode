...
public class CountUnhappyFriends{
    ...
    class Solution {
        public int unhappyFriends(int n, int[][] preferences, int[][] pairs) {
            // 模拟
            return solution1(n, preferences, pairs);
        }

        private int solution1(int n, int[][] preferences, int[][] pairs) {
            boolean[][] unHappy = new boolean[n][n];
            for (int[] pair: pairs) {
                signHappy(pair[0], pair[1], unHappy, preferences);
                signHappy(pair[1], pair[0], unHappy, preferences);
            }

            int ans = 0;
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    if (unHappy[i][j] && unHappy[j][i]){
                        ans ++;
                        break;
                    }
                }
        ...
        private void signHappy(int x, int y, boolean[][] unHappy, int[][] preferences) {
            for (int u: preferences[x]) {
                // 先前遍历出的人，只要不是验证这个人，就不是很开心
                if (u != y){
                    unHappy[x][u] = true;
                } else {
                    break;
