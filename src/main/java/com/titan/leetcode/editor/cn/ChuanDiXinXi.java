...
class Solution {
    public int numWays(int n, int[][] relation, int k) {
        // 递归
        // 目标值n-1, 次数k
        return dfs(n - 1, k, relation, 0, 0, 0);
    }

    private int dfs(int destValue, int times, int[][] relation, int level, int currentValue, int retValue) {
        if (times == level){
        ...
        for (int[] array: relation) {
            if (array[0] == currentValue){
                retValue = dfs(destValue, times, relation, level + 1, array[1], retValue);
            }
        }
